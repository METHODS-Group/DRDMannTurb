...
class generator:
    ...
    def generate(self, eta_ones=False):
        ...
        u1_freq = C1 * noise_hat
        u2_freq = C2 * noise_hat
    ...
    def compute_spectrum(
        self, u1: np.ndarray = None, u2: np.ndarray = None
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Estimates the 1d spectra F11 and F22 of the"""
    ...
    def analytical_spectrum(self, k1_arr: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        # F11: integrate Phi11 over k2

        # F22: integrate Phi22 over k2
