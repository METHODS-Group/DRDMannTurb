...
from drdmannturb.shared import *
...
for i in range(3):
    plt.plot(
        k1_data_pts, kF[i], "-", color=clr[i], label=r"$F_{0:d}$ model".format(i + 1)
    )
    plt.plot(
        k1_data_pts,
        DataValues[:, i, i],
...
with open(filename, "wb") as file:
    pickle.dump(
        [config, opt_params, Data, pb.loss_history_total, pb.loss_history_epochs], file
    )
