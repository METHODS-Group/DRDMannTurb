...
def driver():
    for idx, activ_list in enumerate(
        list(product(consts_exp1.ACTIVATIONS, consts_exp1.ACTIVATIONS))
    ):  # [(nn.SELU(), nn.SELU())]: # zip(consts.ACTIVATIONS, consts.ACTIVATIONS)[0]:
        ...
        plt.savefig(
            config["output_folder"] + "/" + str(activ_list) + "train_history.png",
            format="png",
            dpi=100,
        )
