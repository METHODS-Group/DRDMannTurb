...
class PlotSpectrum:
    ...
    def __init__(self, obj) -> None:
        ...
        if plt_tau: k_norm = torch.norm(k, dim=-1)
        ...
        if self.kF_model_vals is None:  self.kF_model_vals = self.OPS(k1).detach().numpy()
        ...
        if not hasattr(self,'fig'):
            ...
            if plt_tau:
                ...
                self.lines_LT_model, = self.ax[1].plot(k_norm, self.tau_model, '-', label=r'$\tau_{model}$')
