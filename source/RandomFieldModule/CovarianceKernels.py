...
class MaternCovariance(Covariance):
    ...
    def __init__(self, nu, corrlen, angle_anis=None, marg_var=1, **kwargs):
        ...
        ### Operator Theta
        self.Theta, self.detTheta = set_ShapeOperator(self.corrlen, self.angle_anis, self.ndim)
    ...
    def set_NormalizingCoefficient(self): 
        ...
        alpha = nu + d/2
        try:
            default_variance = gamma(nu)/gamma(alpha) * (2*nu / (4*pi))**(d/2) / sqrt(self.detTheta)
        except:
    ...
    def eval(self, *args, nu=None, corrlen=None):
        ...
        if corrlen is None: corrlen = self.corrlen[0]
        if len(args)==1:
            r = args[0]
        elif len(args)==2:
            x = args[0]
    ...
    def eval_sqrt(self, *args, nu=None, corrlen=None):
        ...
        d = self.ndim
        nu_mod = (nu-d/2)/2
        sigma = sqrt( gamma(nu + d/2) / gamma(nu) * (nu/(2*pi)/self.detTheta)**(d/2) )
...
class VonKarmanCovariance(Covariance):
    ...
    def __init__(self, correlation_length, viscous_dissipation_rate, kolmogorov_constant, ndim=3, **kwargs):
        super().__init__(**kwargs)
    ...
    def precompute_Spectrum(self, Frequences):
        ...
        const = self.alpha * (self.epsilon**(2/3)) * (self.corrlen**(17/3)) / (4*np.pi)
...
class MannCovariance(Covariance):
    ...
    def precompute_Spectrum(self, Frequences):
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            ...
            SqrtSpectralTens[0,0,...] = -k3 * tmpTens[1,0,...] + k2 * tmpTens[2,0,...]
            SqrtSpectralTens[0,1,...] = -k3 * tmpTens[1,1,...] + k2 * tmpTens[2,1,...]
            SqrtSpectralTens[0,2,...] = -k3 * tmpTens[1,2,...] + k2 * tmpTens[2,2,...]
            SqrtSpectralTens[1,0,...] =  k3 * tmpTens[0,0,...] - k1 * tmpTens[2,0,...]
            SqrtSpectralTens[1,1,...] =  k3 * tmpTens[0,1,...] - k1 * tmpTens[2,1,...]
            SqrtSpectralTens[1,2,...] =  k3 * tmpTens[0,2,...] - k1 * tmpTens[2,2,...]
            SqrtSpectralTens[2,0,...] = -k2 * tmpTens[0,0,...] + k1 * tmpTens[1,0,...]
            SqrtSpectralTens[2,1,...] = -k2 * tmpTens[0,1,...] + k1 * tmpTens[1,1,...]
            SqrtSpectralTens[2,2,...] = -k2 * tmpTens[0,2,...] + k1 * tmpTens[1,2,...]
...
class UniformShearCovariance(Covariance):
    ...
    def precompute_Spectrum(self, Frequences):
        ...
        C1 = beta * k1**2 * (kk - 2 * k30**2 + beta * k1 * k30) / (kkt * (k1**2 + k2**2))
