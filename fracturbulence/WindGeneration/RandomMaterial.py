...
class RandomMaterial:
    def __init__(
        self, grid_level, ndim=2, verbose=0, levelcut_strategy="abs", **kwargs
    ):
        ...
        self.GRF = GaussianRandomField(
            grid_level=grid_level, ndim=ndim, verbose=verbose, **kwargs
        )
    ...
    def sample(self, noise=None):
        ...
        if self.levelcut_strategy is "abs":
            field = np.abs(field)
    ...
    def generate_samples(
        self, nsamples=1, path=None, output_format="png", append=False
    ):
        ...
        for isample in tqdm(range(nsamples)):
            ...
            if output:
                ...
                if self.ndim == 2 and output_format is "png":
                    self.save_png(phase, filename)
                elif self.ndim == 3 or output_format is "vtk":
                    self.save_vtk(phase, filename)
