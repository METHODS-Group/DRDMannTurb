...
class fde_solve:
    ...
    def __init__(
        ...
        self.z_grid = z_grid
        self.reset_ode(coef)
        self.c, self.d = compute_RationalApproximation_AAA_new(
            alpha, beta, nPoints=1000
        )
    ...
    def reset_ode(self, coef):
        if isinstance(coef, Iterable):
            self.ode_solve = [
                ode_solve(
                    self.dof, coefj, domain_height=self.domain_height, grid=self.z_grid
                )
                for coefj in coef
            ]
            self.anisotrop = True
            ...
            self.ode_solve = ode_solve(
                self.dof, coef, domain_height=self.domain_height, grid=self.z_grid
            )
    ...
    def reset_jac(self, grad_coef):
        self.grad_term1 = [
            self.alpha * der_coef(self.grid) / self.coef(self.grid)
            for der_coef in grad_coef
        ]
        self.gradA = [
            ode_solve(self.dof, der_coef, domain_height=self.domain_height)
            for der_coef in grad_coef
        ]
...
def func(args):
    i, self = args
    if self.anisotrop:
        return self.ode_solve[self.component](
            self.d[i],
            self.rhs,
            self.k[0],
            self.k[1],
            Robin_const=self.Robin_const,
            **self.kwargs
        )
    else:
...
def func_assemble(args):
    i, self = args
    gradA_mode = self.gradA[self.comp].apply_matvec(
        -1, self.modes[i], self.k[0], self.k[1]
    )
