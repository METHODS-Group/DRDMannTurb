...
class ExponentialExpansion(BasicExpansion):
    def __init__(self, **kwargs):
        ...
        self.Gradient = [
            ExpansionDerivative(self, comp) for comp in range(self.c.size + self.d.size)
        ]
    ...
    def eval(self, z):
        if self.c.size == 0:
            ...
            return (
                self.L_inf_2
                * (1 + np.sum(self.c * np.exp(-self.d**2 * z / self.L_inf), axis=0))
                ** 2
            )
    ...
    def Derivative(self, z, comp):
        if comp < self.c.size:
            ...
            return (
                self.L_inf_2
                * np.exp(-self.c[i] ** 2 * (z + self.d[i] ** 2))
                * 2
                * self.c[i]
                * (z + self.d[i] ** 2)
            )
...
class PolyExpExpansion(BasicExpansion):
    ...
    def __call__(self, z):
        return self.L_inf_2 * (
            1 - self.sigma**2 * np.exp(-self.polyval(z / self.rho**2))
        )
    ...
    def Derivative(self, z, comp):
        if comp is 0:
            return (
                -2
                * self.sigma
                * np.exp(-self.polyval(z / self.rho**2))
                * self.L_inf_2
            )
        elif comp is 1:
            return (
                ...
                * np.exp(-self.polyval(z / self.rho**2))
                * self.L_inf_2
                * (self.polyval_der1(z / self.rho**2) * 2 / self.rho)
            )
        elif comp >= 2:
            i = comp - 2
            k = self.poly.size - i
...
class PowerExpExpansion(BasicExpansion):
    ...
    def __call__(self, z):
        return self.L_inf_2 * (
            1 - self.sigma**2 * np.exp(-((z / self.rho) ** self.nu))
        )
    ...
    def Derivative(self, z, comp):
        if comp is 0:
            ...
            return (
                -self.sigma**2
                * np.exp(-((z / self.rho) ** self.nu))
                * self.L_inf_2
                * (self.nu * (z / self.rho) ** (self.nu - 1) / self.rho**2)
            )
        elif comp is 2:
            return (
                -self.sigma**2
                * np.exp(-((z / self.rho) ** self.nu))
                * self.L_inf_2
                * ((z / self.rho) ** (self.nu) * np.log(z / self.rho))
            )
        else:
            raise Exception("Unkown derivative component!")
...
class MultiPoleExpansion(BasicExpansion):
    ...
    def __call__(self, z):
        return (
            self.L_inf_2
            * (1 + np.sum(self.c / (z[:, None] / self.L_inf + self.d**2), axis=1))
            ** 2
        )
