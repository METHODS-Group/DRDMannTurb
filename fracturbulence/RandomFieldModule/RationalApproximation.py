...
def compute_RationalApproximation_AAA(
    alpha, MaxOrder=100, tol=1.0e-8, nPoints=1000, verbose=False
):
    ...
    E = np.block(
        [[0, (w * f).reshape([1, m])], [np.ones([m, 1]), np.diag(z.flatten())]]
    )
...
def compute_RationalApproximation_AAA_new(
    alpha, beta=None, MaxOrder=100, tol=1.0e-12, nPoints=1000, verbose=False
):
    # deal with divisions by zero
    np.seterr(divide="ignore", invalid="ignore")
    ...
    M = nPoints
    Z = np.linspace(1 + 1.0e-3, 1000, M).reshape([M, 1])
    M = Z.size
    F = func(Z)
    SF = scipy.sparse.spdiags(F.flatten(), 0, M, M)
    J = np.arange(M)
...
if __name__ == "__main__":
    ...
    c, d = compute_RationalApproximation_AAA(
        alpha, MaxOrder=MaxOrder, tol=tol, nPoints=nPoints, verbose=False
    )

    func = lambda x: x ** (-alpha)
    appx = lambda x: np.sum([c[k] / (x + d[k]) for k in range(c.size)])

    x = np.geomspace(1, 1.0e8, 100000)
