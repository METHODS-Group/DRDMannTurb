...
class DerivativeStiffnessMatrixGenerator(DerivativeMatrixGenerator):
    ...
    def __init__(
        self, dof, L_coef, EddyLifetime, domain_height=1.0, sqrtRobin_const=0.0
    ):
    ...
    def assemble_derivative(self, k1, k2):
        ...
        m = len(self.grid)
        dA11dtau = (
            np.diag(tmp1[0, 1:], k=1)
            + np.diag(tmp1[1, :], k=0)
            + np.diag(tmp1[2, :-1], k=-1)
        )
        ...
        tmp2 = (
            2.0 * tmp2 / self.lengthscale
        )  # trick assuming L^2(z) = lengthscale^2 * z^beta
        dA11dlengthscale = (
            np.diag(tmp2[0, 1:], k=1)
            + np.diag(tmp2[1, :], k=0)
            + np.diag(tmp2[2, :-1], k=-1)
        )
        ...
        return (
            [dAdlengthscale]
            + [self.dAdscalingfactor]
            + [self.dAdsqrtRobin_const]
    ...
    def mass_matrix(self, reaction_function):
        ...
        if np.isscalar(fun_qp):
            fun_qp *= np.ones_like(self.qp)
        w = 1 / 2
        x1 = (1 - 1 / np.sqrt(3)) / 2
        x2 = (1 + 1 / np.sqrt(3)) / 2
        coeff_element_0 = (
            (fun_qp[0, :] * (x1**2) + fun_qp[1, :] * (x2**2)) * self.grid.h * w
        )
        coeff_element_1 = (
            (fun_qp[0, :] * (x1 * (1 - x1)) + fun_qp[1, :] * (x2 * (1 - x2)))
            * self.grid.h
            * w
        )
...
class DerivativeCovarianceMatrixGenerator(DerivativeMatrixGenerator):
    ...
    def compute_fourier_basis(self):
        ...
        for l in range(M):
            phi_hat[l, :] = (
                np.sinc(h * self.k3 / (2 * np.pi)) ** 2
                * np.exp(-1j * h * l * self.k3)
                * h
                / np.sqrt(2 * np.pi)
            )
    ...
    def assemble_matrix(self, k1, k2, multiply_by_detTheta=True):
        ...
        if multiply_by_detTheta:
            alpha = 17 / 12
            detTheta = (
                self.L_coef[0](self.grid[:])
                * self.L_coef[1](self.grid[:])
                * self.L_coef[2](self.grid[:])
            )
        ...
        G[0, 0, :] = 1
        G[0, 1, :] = 0
        G[0, 2, :] = -zeta1_by_zeta3
        G[1, 0, :] = 0
        G[1, 1, :] = 1
        G[1, 2, :] = -zeta2_by_zeta3
        G[2, 0, :] = G[0, 2, :]
        G[2, 1, :] = G[1, 2, :]
        G[2, 2, :] = zeta1_by_zeta3**2 + zeta2_by_zeta3**2 + one_by_zeta3**2
        ...
        for i in range(3):
            for j in range(3):
                for l in range(M):
                    for m in range(M):
                        B[i * M + l, j * M + m] = np.sum(
                            phi_hat[l, :] * G[i, j, :] * phi_hat[m, :].conj()
                        )
                if multiply_by_detTheta:
                    for l in range(M):
    ...
    def assemble_derivative(self, k1, k2, multiply_by_detTheta=True):
        ...
        dG33dtau = (
            2.0 * (zeta1 * dzeta1dtau + zeta2 * dzeta2dtau) / zeta3**2
        ...
        for l in range(M):
            for m in range(M):
                dBdtau[i * M + l, j * M + m] = np.sum(
                    phi_hat[l, :] * dG13dtau * phi_hat[m, :].conj()
                )
        ...
        for l in range(M):
            for m in range(M):
                dBdtau[i * M + l, j * M + m] = np.sum(
                    phi_hat[l, :] * dG23dtau * phi_hat[m, :].conj()
                )
        ...
        for l in range(M):
            for m in range(l, M):
                dBdtau[i * M + l, j * M + m] = np.sum(
                    phi_hat[l, :] * dG33dtau * phi_hat[m, :].conj()
                )
...
if __name__ == "__main__":
    ...
    hidden_layer_size = 2
    noise_magnitude = 1.0
    EddyLifetimeGrid = EddyLifetimeGrid(
        frequencies,
        tau0=tau0,
    ...
    L3 = lambda z: z**3
    coef = kappa = [
        np.vectorize(lambda z: L1(z) ** 2),
        np.vectorize(lambda z: L2(z) ** 2),
        np.vectorize(lambda z: L3(z) ** 2),
    ]

    domain_height = 0.9
    dof = 2**6
    DerivativeStiffnessMatrixGenerator = DerivativeStiffnessMatrixGenerator(
        dof, coef, EddyLifetime, domain_height=domain_height
    )
    DerivativeCovarianceMatrixGenerator = DerivativeCovarianceMatrixGenerator(
        dof, coef, EddyLifetime, domain_height=domain_height
    )
