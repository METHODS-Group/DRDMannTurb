...
class GenericObjectiveFunction:
    ...
    def plot(self, dynamic=False):
        ...
        if not hasattr(self,'fig'):
            self.fig, self.ax = subplots(nrows=1, ncols=2, num='Calibration', clear=True, figsize=[20, 10])
            ...
            for i in range(self.vdim):
                self.lines_SP_model[i], = self.ax[0].plot(k1, self.SP[:,i,i], 'o-', label='F{0:d} model'.format(i))
            for i in range(self.vdim):
                self.lines_SP_data[i],  = self.ax[0].plot(k1, self.DataValues[:,i,i], '--', label='F{0:d} data'.format(i))
            self.ax[0].legend()
            ...
            self.tau_ref   = k**(-2/3)
            self.lines_LT_model, = self.ax[1].plot(k, self.tau_model, '-', label=r'$\tau_{model}$')
            self.lines_LT_ref,   = self.ax[1].plot(k, self.tau_ref,  '--', label=r'$\tau_{ref}=|k|^{-\frac{2}{3}}$')
            self.ax[1].legend()
