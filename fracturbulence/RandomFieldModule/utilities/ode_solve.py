...
class FEM_coefficient_matrix_generator:
    ...
    def __init__(self, grid, coef):
        ...
        det = (
            self.coef[0](self.grid[:])
            * self.coef[1](self.grid[:])
            * self.coef[2](self.grid[:])
        ) ** (1 / 3)
    ...
    def __call__(self, d, k1, k2, t=0.0):
        ...
        crossterm_diffusion_function = self._crossterm_diffusion_function(
            self.coef, k1, t
        )
        # create the matrices from the coefficient functions
...
class FEM_load_vector_generator:
    ...
    def _delta_load_smooth(self, z, epsilon=1e-2, order=5):
        ...
        for i in range(order):
            coeff_shape_function_left[ind] += (
                fun_qp[i] * (1 - x_qp_right[i]) * w[i] * epsilon
            )
            coeff_shape_function_right[ind] += (
                fun_qp[i] * x_qp_right[i] * w[i] * epsilon
            )
            if ind > 1:
                coeff_shape_function_left[ind - 1] += (
                    fun_qp[i] * x_qp_left[i] * w[i] * epsilon
                )
                coeff_shape_function_right[ind - 1] += (
                    fun_qp[i] * (1 - x_qp_left[i]) * w[i] * epsilon
                )
    ...
    def mollifier(epsilon, z0=0.0):
        return (
            lambda z: 0.0
            if (z - z0) ** 2 >= epsilon**2
            else np.exp(-(epsilon**2) / (epsilon**2 - (z - z0) ** 2))
        )
...
class ode_solve:
    def __init__(self, dof, coef, domain_height=1, grid=None):
        ...
        self.FEM_coefficient_matrix_generator = FEM_coefficient_matrix_generator(
            self.grid, coef
        )
    ...
    def __call__(self, d, f, k1, k2, t=0.0, Robin_const=None, adjoint=False, **kwargs):
        ...
        if test:
            ...
            if callable(f):
                ...
                rhs = (
                    1 * f
                )  ### variant of copying vector (important fot not modifying the input)
            # if not adjoint:
            #     rhs = 1*f
...
def quadrature_weights_local(order):
    ...
    if order == 4:
        w = (
            np.array(
                [18 - np.sqrt(30), 18 + np.sqrt(30), 18 + np.sqrt(30), 18 - np.sqrt(30)]
            )
            / 36
        )
    if order == 5:
...
if __name__ == "__main__":
    ...
    f = lambda z: (
        (
            d
            + 1.0
            + (L1(z) ** 2 + t**2 * L3(z) ** 2) * k1**2
            + L2(z) ** 2 * k2**2
            - (1j * t * k1) * (2 * L3(z) * dL3dz(z))
        )
        * psi(z)
        - 2.0 * (1j * t * k1 * L3(z) ** 2 + L3(z) * dL3dz(z)) * dpsidz(z)
        - L3(z) ** 2 * d2psidz2(z)
    )
